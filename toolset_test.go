...
func TestEveryRegisteredCommand(t *testing.T) {
	tests := []struct {
		...
		{name: "gunzip", script: `printf hello | gzip -c >/a.gz; gunzip -c /a.gz`, want: "hello"},
		{name: "gzip", script: `printf hello | gzip -c | gunzip -c`, want: "hello"},
		{name: "head", script: `printf 'a\nb\n' | head -n1`, want: "a\n"},
		{name: "hexdump", script: `printf A | hexdump -C`, want: "00000000  41                                                |A|\n00000001\n"},
		...
		{name: "od", script: `printf A | od -An -tx1`, want: " 41\n"},
		...
		{name: "tar", script: `printf x >/a; tar -cf /a.tar /a; tar -tf /a.tar`, want: "a\n"},
		...
		{name: "unzip", script: `printf x >/a; zip -q /a.zip /a; unzip -Z1 /a.zip`, want: "a\n"},
		{name: "wc", script: `printf 'a b\n' | wc -w`, want: "2 \n"},
		{name: "whoami", script: `whoami`, want: "agent\n"},
		{name: "xargs", script: `printf 'a b' | xargs`, want: "a b\n"},
		{name: "xxd", script: `printf A | xxd -p`, want: "41\n"},
		{name: "zip", script: `printf x >/a; zip -q /a.zip /a; unzip -p /a.zip`, want: "x"},
	}
